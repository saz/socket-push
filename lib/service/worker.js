...
function Worker(nodeId, serviceFactory, configService) {
    ...
    function createClientPort(config) {
        if (clientPort === undefined) {
            var connect = require('connect'), server;

            if (config.clientPort.useSSL === true && typeof config.clientPort.sslKey === 'string' && typeof config.clientPort.sslCert === 'string') {
                logger.info('Secure connection active');
                var fs = require('fs');
                server = connect(
                    {   key: fs.readFileSync(config.clientPort.sslKey),
                        cert: fs.readFileSync(config.clientPort.sslCert)
                    },
                    connect.logger(),
